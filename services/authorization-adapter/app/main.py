...
def current_principal(
    ...
    return Principal(
        ...
        auth_strategy="kratos-session-or-dev-header",
...
def ready(settings: Annotated[Settings, Depends(get_settings)]) -> ReadinessResponse:
    checks = {
        "configuration": "ok",
        "token_secret": (
            "dev-placeholder" if settings.token_secret == "replace-with-dev-secret" else "ok"
        ),
...
def permissions() -> PermissionCatalog:
    return PermissionCatalog(roles=ROLE_SCOPES, default_role="owner")
